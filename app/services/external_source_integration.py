    ...
    def __post_init__(self):
        ...
        if self.auth_type == AuthType.API_TOKEN:
            if not self.api_token or not self.username:
                raise ValueError("api_token and username are required for API token auth")
...
class ConfluenceConnector(BaseConnector):
    ...
    async def fetch_documents(self, incremental: bool = False) -> List[Dict[str, Any]]:
        ...
        for page in pages:
            try:
                ...
                logger.warning(f"Failed to transform page {page.get('id', 'unknown')}: {e}")
                continue
    ...
    async def _fetch_pages_batch(self, start: int, limit: int, incremental: bool = False) -> List[Dict[str, Any]]:
        """ページのバッチを取得"""
        params = {
            "start": start,
            "limit": limit,
    ...
    def _transform_page_to_document(self, page: Dict[str, Any]) -> Dict[str, Any]:
        ...
        metadata = {
            ...
            "page_url": urljoin(self.config.base_url, page.get("_links", {}).get("webui", "")),
...
class JiraConnector(BaseConnector):
    ...
    async def fetch_documents(self, incremental: bool = False) -> List[Dict[str, Any]]:
        ...
        for issue in issues:
            try:
                ...
                logger.warning(f"Failed to transform issue {issue.get('key', 'unknown')}: {e}")
                continue
        
        return documents
    
    async def _fetch_all_issues(self, incremental: bool = False) -> List[Dict[str, Any]]:
        ...
        while len(all_issues) < self.config.max_issues:
            issues_batch = await self._fetch_issues_batch(start_at, max_results, incremental)
    ...
    async def _fetch_issues_batch(self, start_at: int, max_results: int, incremental: bool = False) -> List[Dict[str, Any]]:
        """課題のバッチを取得"""
        jql_parts = []
        ...
        payload = {
            ...
            "fields": [
                ...
                "project", "assignee", "reporter", "created", "updated",
    ...
    def _transform_issue_to_document(self, issue: Dict[str, Any]) -> Dict[str, Any]:
        ...
        content_parts.append(f"Issue Type: {fields.get('issuetype', {}).get('name', 'Unknown')}")
        content_parts.append(f"Status: {fields.get('status', {}).get('name', 'Unknown')}")
        
        if fields.get("priority"):
            content_parts.append(f"Priority: {fields['priority'].get('name', 'Unknown')}")
        
        if fields.get("assignee"):
            content_parts.append(f"Assignee: {fields['assignee'].get('displayName', 'Unknown')}")
        
        if fields.get("reporter"):
            content_parts.append(f"Reporter: {fields['reporter'].get('displayName', 'Unknown')}")
    ...
    async def _process_document_batch(self, documents: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        ...
        for doc in documents:
            try:
                ...
                logger.warning(f"Failed to process document {doc.get('id', 'unknown')}: {e}")
                continue
