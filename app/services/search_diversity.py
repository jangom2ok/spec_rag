...
class DiversificationRequest:
    ...
    diversification_criteria: List[str] = field(default_factory=lambda: ["topic", "category"])
...
class BaseDiversifier:
    ...
    async def select(self, candidates: List[DiversityCandidate], **kwargs) -> List[DiversityCandidate]:
        """多様化選択（オーバーライド必須）"""
...
class MMRDiversifier(BaseDiversifier):
    ...
    async def select(
        ...
        # 最初は最高スコアの候補を選択
        best_candidate = max(remaining, key=lambda x: x.score)
    ...
    def _calculate_mmr_score(
        ...
        mmr_score = diversity_factor * relevance_score - (1 - diversity_factor) * max_similarity
        return mmr_score
    
    def _calculate_similarity(self, candidate1: DiversityCandidate, candidate2: DiversityCandidate) -> float:
        """2つの候補間の類似度計算"""
        if candidate1.embedding is not None and candidate2.embedding is not None:
            ...
            similarity = np.dot(candidate1.embedding, candidate2.embedding) / (
                np.linalg.norm(candidate1.embedding) * np.linalg.norm(candidate2.embedding)
            )
    ...
    async def _calculate_similarity_matrix(self, candidates: List[DiversityCandidate]) -> np.ndarray:
        """類似度行列計算"""
        n = len(candidates)
...
class ClusteringDiversifier(BaseDiversifier):
    ...
    async def select(
        ...
        # 各クラスターから代表候補を選択
        selected = []
        clusters_sorted = sorted(clusters, key=lambda c: len(c.candidates), reverse=True)
    ...
    async def _perform_kmeans_clustering(
        ...
        # K-meansクラスタリング
        try:
            kmeans = KMeans(n_clusters=min(k, len(candidates)), random_state=42, n_init=10)
        ...
        for cluster_id in range(k):
            cluster_candidates = [
                candidates[i] for i, label in enumerate(cluster_labels) 
                if label == cluster_id
            ]
            
            if cluster_candidates:
                centroid = centroids[cluster_id] if cluster_id < len(centroids) else np.mean(
                    [c.embedding for c in cluster_candidates], axis=0
                )
    ...
    def _calculate_cluster_coherence(self, candidates: List[DiversityCandidate]) -> float:
        """クラスターコヒーレンススコア計算"""
        if len(candidates) <= 1:
            return 1.0
...
class TopicDiversifier(BaseDiversifier):
    ...
    async def select(
        ...
        while len(selected) < max_results and remaining:
            ...
            for candidate in remaining:
                ...
                total_score = (1 - topic_weight) * candidate.score + topic_weight * topic_novelty
    ...
    async def _extract_topics(self, candidates: List[DiversityCandidate]) -> Dict[str, List[str]]:
        """候補からトピック抽出"""
        topics_dict = {}
        ...
        for candidate in candidates:
            ...
            if "topics" in candidate.metadata:
                ...
                # テキストからトピック抽出
                extracted_topics = await self._extract_topics_from_content(candidate.content)
    ...
    def _calculate_topic_diversity_score(self, topics_dict: Dict[str, List[str]]) -> float:
        """トピック多様性スコア計算"""
        all_topics = set()
...
class TemporalDiversifier(BaseDiversifier):
    ...
    async def select_temporally_diverse(
        ...
        if len(selected) < max_results:
            remaining_candidates = [
                c for c, _ in candidates_with_dates
                if c not in selected
            ]
            remaining_slots = max_results - len(selected)
            selected.extend(remaining_candidates[:remaining_slots])
...
class SearchDiversityService:
    ...
    def _create_diversifiers(self) -> Dict[DiversificationAlgorithm, BaseDiversifier]:
        ...
        diversifiers[DiversificationAlgorithm.CLUSTERING] = ClusteringDiversifier(self.config)
        diversifiers[DiversificationAlgorithm.TOPIC_BASED] = TopicDiversifier(self.config)
        diversifiers[DiversificationAlgorithm.TEMPORAL] = TemporalDiversifier(self.config)
    ...
    async def diversify(self, request: DiversificationRequest) -> DiversificationResult:
        ...
        try:
            ...
            if request.preserve_top_results > 0:
                sorted_candidates = sorted(request.candidates, key=lambda x: x.score, reverse=True)
            ...
            if self.config.algorithm == DiversificationAlgorithm.MMR:
                diversified = await self._diversify_with_mmr(
                    remaining_candidates, max_results - len(preserved_candidates), diversity_factor
                )
            elif self.config.algorithm == DiversificationAlgorithm.CLUSTERING:
                diversified = await self._diversify_with_clustering(
                    remaining_candidates, max_results - len(preserved_candidates), self.config.cluster_count
                )
            elif self.config.algorithm == DiversificationAlgorithm.TOPIC_BASED:
                diversified = await self._diversify_with_topics(
                    remaining_candidates, max_results - len(preserved_candidates), self.config.topic_weight
                )
            elif self.config.algorithm == DiversificationAlgorithm.TEMPORAL:
                diversified = await self._diversify_with_temporal(
                    remaining_candidates, max_results - len(preserved_candidates), self.config.temporal_window_days
                )
            else:
                # デフォルトでMMRを使用
                diversified = await self._diversify_with_mmr(
            ...
            # 多様性メトリクス計算
            diversity_metrics = self._calculate_diversity_metrics(final_candidates, request.candidates)
    ...
    async def _diversify_with_temporal(
        ...
        return await diversifier.select_temporally_diverse(candidates, max_results, time_window_days)
    ...
    def _calculate_diversity_metrics(
        ...
        for i in range(len(selected_candidates)):
            for j in range(i + 1, len(selected_candidates)):
                similarity = self._calculate_similarity(selected_candidates[i], selected_candidates[j])
                dissimilarity = 1.0 - similarity
                total_dissimilarity += dissimilarity
                pairs_count += 1
        
        intra_list_diversity = total_dissimilarity / pairs_count if pairs_count > 0 else 0.0
        ...
        coverage_ratio = len(selected_categories) / len(original_categories) if original_categories else 0.0
        ...
        novelty_score = len(selected_topics) / len(selected_candidates) if selected_candidates else 0.0
        ...
        for candidate in candidates:
            ...
            for criterion in criteria:
                if criterion == "category":
                    # カテゴリ多様性
                    category = candidate.metadata.get("category", "unknown")
                    scores["category"] = self._calculate_category_diversity_score(candidate, candidates)
                elif criterion == "author":
                    # 著者多様性
                    author = candidate.metadata.get("author", "unknown")
                    scores["author"] = self._calculate_author_diversity_score(candidate, candidates)
                elif criterion == "topic":
                    # トピック多様性
                    topics = candidate.metadata.get("topics", [])
                    scores["topic"] = self._calculate_topic_diversity_score_for_candidate(candidate, candidates)
    ...
    def _calculate_category_diversity_score(
        self, 
        candidate: DiversityCandidate, 
        all_candidates: List[DiversityCandidate]
    ) -> float:
        """カテゴリ多様性スコア計算"""
    ...
    def _calculate_topic_diversity_score_for_candidate(
        ...
        diversity_score = 1.0 - (total_overlap / max_possible_overlap) if max_possible_overlap > 0 else 1.0
    ...
    async def _get_from_cache(self, cache_key: str) -> Optional[DiversificationResult]:
        """キャッシュから取得"""
