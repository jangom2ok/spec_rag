...
class StructureExtractor:
    ...
    def extract_html_structure(content: str) -> Dict[str, Any]:
        ...
        structure["elements"] = []
        if structure["lists"]:
            structure["elements"].extend([{"type": "list", "count": len(structure["lists"])}])
        if structure["tables"]:
            structure["elements"].extend([{"type": "table", "count": len(structure["tables"])}])
...
class EntityExtractor:
    ...
    @staticmethod
    def extract_entities(content: str, confidence_threshold: float = 0.5) -> Dict[str, Any]:
        """エンティティを抽出"""
        entities = {
            "technical_terms": [],
            "http_codes": [],
        ...
        tech_terms = re.findall(r'\b[A-Z]{2,}\b|API|HTTP|JSON|XML|SQL|REST|SOAP', content)
...
class KeywordExtractor:
    ...
    @staticmethod
    def extract_keywords(content: str, max_keywords: int = 20, language: str = "en") -> List[Dict[str, Any]]:
        """キーワードを抽出"""
        # ストップワード（簡易版）
        stop_words_en = {
        ...
        stop_words_ja = {
            ...
            'その後', '方', '今', '前', '後', '間', '上', '下', '中', '内', '外', '左', '右', '先', '奥',
            '手', '本', '事', '物', '人', '者', '方', '時', '日', '年', '月', '週', '国', '家', '会社'
        }
...
class StatisticsExtractor:
    ...
    @staticmethod
    def extract_statistics(content: str, structure: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
        """統計情報を抽出"""
        lines = content.split('\n')
        ...
        # 複雑度指標
        avg_sentence_length = basic_stats["word_count"] / max(basic_stats["sentence_count"], 1)
        ...
        try:
            ...
            if self.config.extract_structure:
                if doc_type == DocumentType.MARKDOWN:
                    structure = self.structure_extractor.extract_markdown_structure(content)
            ...
            if self.config.extract_statistics:
                statistics = self.statistics_extractor.extract_statistics(content, structure)
