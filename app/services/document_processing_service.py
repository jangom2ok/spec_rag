...
from app.services.document_collector import DocumentCollector, CollectionConfig, SourceType
from app.services.metadata_extractor import MetadataExtractor, ExtractionConfig
...
class DocumentProcessingService:
    ...
    async def process_documents(self, config: ProcessingConfig) -> ProcessingResult:
        ...
        try:
            ...
            self._stage_times[ProcessingStage.COLLECTION] = (datetime.now() - stage_start).total_seconds()
    ...
    async def _collect_documents(self, config: CollectionConfig) -> List[Dict[str, Any]]:
        """ドキュメントを収集"""
    ...
    async def _process_single_document(
        ...
        async with doc_semaphore:
            ...
            try:
                ...
                # Stage 2: メタデータ抽出
                metadata_result = await self._extract_metadata(document, config.extraction_config)
                if metadata_result:
    ...
    async def _extract_metadata(
        ...
        try:
            ...
            if result.success:
                ...
                logger.warning(f"Metadata extraction failed for {document.get('id')}: {result.error_message}")
                return None
    ...
    async def reprocess_failed_documents(self, config: ProcessingConfig) -> ProcessingResult:
        """失敗したドキュメントを再処理"""
        try:
            # 失敗状態のドキュメントIDを取得
            failed_ids = [
                doc_id for doc_id, status in self._processing_status.items()
            ...
            # 結果の集計
            successful_docs = sum(1 for r in results if isinstance(r, dict) and r.get("success", False))
