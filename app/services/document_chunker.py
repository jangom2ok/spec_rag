...
class ChunkResult:
    ...
    def get_summary(self) -> Dict[str, Any]:
        ...
        average_chunk_size = total_content_length / len(self.chunks) if self.chunks else 0
...
class FixedSizeChunker:
    ...
    async def chunk(self, document: Dict[str, Any]) -> List[DocumentChunk]:
        ...
        if self.config.preserve_paragraphs:
            ...
            for paragraph in paragraphs:
                ...
                if len(test_chunk) <= self.config.chunk_size:
                    ...
                    if self.config.overlap_size > 0 and current_chunk:
                        overlap_text = current_chunk[-self.config.overlap_size:].strip()
                        if overlap_text:
                    ...
                    if len(current_chunk) > self.config.chunk_size:
                        ...
                        chunk_index += 1
                        current_chunk = current_chunk[self.config.chunk_size - self.config.overlap_size:]
            ...
            for sentence in sentences:
                ...
                # 文を追加できるかチェック
                test_chunk = current_chunk + (" " if current_chunk else "") + sentence_clean
            ...
            for i in range(0, len(content), step_size):
                ...
                if len(chunk_content) >= self.config.min_chunk_size or i + self.config.chunk_size >= len(content):
                    chunks.append(self._create_chunk(
                        document_id, chunk_index, chunk_content, language
                    ))
                    chunk_index += 1
    ...
    def _create_chunk(self, document_id: str, chunk_index: int, content: str, language: str) -> DocumentChunk:
        """チャンクを作成"""
...
class SemanticChunker:
            ...
            for section_title, section_content in sections:
                if len(section_content) > self.config.chunk_size:
                    ...
                    sub_chunks = await self._split_large_section(
                        section_content, document_id, chunk_index, language, section_title
                    )
                    chunks.extend(sub_chunks)
                    chunk_index += len(sub_chunks)
                else:
                    chunk = self._create_semantic_chunk(
                        document_id, chunk_index, section_content, language, 
                        section_title, ChunkType.SECTION
                    )
                    chunks.append(chunk)
                    chunk_index += 1
        else:
    ...
    async def _split_large_section(self, content: str, document_id: str, start_index: int, 
                                 language: str, section_title: str) -> List[DocumentChunk]:
        """大きなセクションを分割"""
        # 固定サイズチャンカーを使用して分割
    ...
    def _create_semantic_chunk(self, document_id: str, chunk_index: int, content: str, 
                             language: str, title: Optional[str] = None, 
                             chunk_type: ChunkType = ChunkType.TEXT) -> DocumentChunk:
        """セマンティックチャンクを作成"""
        chunk_id = str(uuid.uuid4())
...
class HierarchicalChunker:
    ...
    def _build_hierarchy(self, headings: List[Tuple[str, int, str]], content: str) -> List[Dict[str, Any]]:
        """階層構造を構築"""
        if not headings:
            return [{
                "title": "",
        ...
        for i, (title, level, heading_line) in enumerate(headings):
            # セクションの開始位置を見つける
            start_idx = next((j for j, line in enumerate(lines) if line.strip() == heading_line), 0)
            
            # セクションの終了位置を見つける
            end_idx = len(lines)
            if i + 1 < len(headings):
                next_heading_line = headings[i + 1][2]
                end_idx = next((j for j, line in enumerate(lines) if line.strip() == next_heading_line), len(lines))
    ...
    def _build_path(self, existing_sections: List[Dict[str, Any]], title: str, level: int) -> str:
        """階層パスを構築"""
        path_parts = []
    ...
    async def _chunk_section(self, section: Dict[str, Any], document_id: str, 
                           start_index: int, language: str) -> List[DocumentChunk]:
        """セクションをチャンク化"""
