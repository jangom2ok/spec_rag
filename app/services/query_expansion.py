...
class QueryExpansionConfig:
    ...
    expansion_methods: List[ExpansionMethod] = field(default_factory=lambda: [ExpansionMethod.SYNONYM])
...
class ExpansionResult:
    ...
    expanded_query_obj: Optional[ExpandedQuery] = None
...
class BaseExpander:
    ...
    async def expand(self, terms: List[str], language: str = "en", **kwargs) -> Dict[str, List[str]]:
        """拡張実行（オーバーライド必須）"""
...
class SemanticExpander(BaseExpander):
    ...
    async def expand_phrase(self, phrase: str, max_expansions: int = 5) -> List[Tuple[str, float]]:
        """フレーズのセマンティック拡張"""
        if not self.model:
        ...
        # 閾値でフィルタリング
        filtered_phrases = [
            (phrase, score) for phrase, score in similar_phrases
            if score >= self.similarity_threshold
        ]
        
        return filtered_phrases
    
    def _find_similar_phrases(self, phrase: str, max_expansions: int) -> List[Tuple[str, float]]:
        """類似フレーズ検索（モック実装）"""
...
class ConceptualExpander(BaseExpander):
    ...
    async def expand_concept(self, concept: str, domain: str = None) -> Dict[str, List[str]]:
        """概念拡張"""
    ...
    def _get_concept_hierarchy(self, concept: str, domain: str = None) -> Dict[str, List[str]]:
        """概念階層取得（モック実装）"""
        # 実際の実装では ConceptNet, WordNet, ドメイン知識ベースを使用
        hierarchy_dict = {
            "machine_learning": {
                "broader": ["artificial_intelligence", "computer_science"],
                "narrower": ["supervised_learning", "unsupervised_learning", "reinforcement_learning"],
                "related": ["data_mining", "pattern_recognition", "statistical_learning"],
            },
            "algorithms": {
                "broader": ["computer_science", "mathematics"],
                "narrower": ["sorting_algorithms", "search_algorithms", "optimization_algorithms"],
                "related": ["data_structures", "complexity_theory", "computational_methods"],
            },
            "neural_network": {
                "broader": ["artificial_intelligence", "machine_learning"],
                "narrower": ["convolutional_neural_network", "recurrent_neural_network"],
        ...
        return hierarchy_dict.get(concept, {"broader": [], "narrower": [], "related": []})
...
class StatisticalExpander(BaseExpander):
    ...
    async def get_co_occurrence_terms(self, term: str, min_frequency: int = None) -> List[Tuple[str, float, int]]:
        """共起語取得"""
        ...
        # 最小頻度でフィルタリング
        filtered_terms = [
            (term, pmi, freq) for term, pmi, freq in cooccur_terms
            if freq >= min_freq
        ]
...
class QueryExpansionService:
    ...
    async def expand_query(self, request: ExpansionRequest) -> ExpansionResult:
        ...
        try:
            ...
            # クエリ用語抽出
            query_terms = await self._extract_query_terms(request.query, request.language)
            ...
            if ExpansionMethod.CONCEPTUAL in self.config.expansion_methods:
                conceptual_expansions = await self._expand_with_concepts(
                    [term.term.replace(" ", "_") for term in query_terms], request.domain
                )
    ...
    async def _extract_query_terms(self, query: str, language: str = "en") -> List[QueryTerm]:
        """クエリ用語抽出"""
    ...
    async def _expand_with_synonyms(self, terms: List[str], language: str) -> Dict[str, List[str]]:
        """同義語拡張"""
    ...
    async def _expand_with_semantics(self, query: str, threshold: float) -> Dict[str, List[Tuple[str, float]]]:
        """セマンティック拡張"""
        ...
        for phrase in phrases:
            similar_phrases = await semantic_expander.expand_phrase(phrase, self.config.max_expansions)
            if similar_phrases:
        ...
        for word in words:
            if len(word) > 2:  # 短い単語はスキップ
                similar_phrases = await semantic_expander.expand_phrase(word, self.config.max_expansions)
                if similar_phrases:
                    semantics[word] = similar_phrases
        
        return semantics
    
    async def _expand_with_concepts(self, terms: List[str], domain: str = None) -> Dict[str, Dict[str, List[str]]]:
        """概念拡張"""
    ...
    async def _expand_with_statistics(self, query: str, min_frequency: int) -> Dict[str, List[Tuple[str, float, int]]]:
        """統計的拡張"""
        ...
        for word in words:
            if len(word) > 2:
                cooccur_terms = await statistical_expander.get_co_occurrence_terms(word, min_frequency)
                if cooccur_terms:
        ...
        if len(words) > 1:
            phrase = " ".join(words)
            cooccur_terms = await statistical_expander.get_co_occurrence_terms(phrase, min_frequency)
            if cooccur_terms:
        ...
        for term, expansions in all_expansions.items():
            if isinstance(expansions, list):
                if expansions and isinstance(expansions[0], tuple):
                    # (term, score) の形式
                    filtered_list = [
                        (exp_term, score) for exp_term, score in expansions
                        if score >= threshold
                    ]
                    # スコア降順でソート
    ...
    def _generate_expanded_query(
        ...
        return ExpandedQuery(
            ...
            expansion_methods_used=[method.value for method in self.config.expansion_methods],
        )
    ...
    def _calculate_expansion_weights(
        ...
        # 拡張用語の重み
        expansion_weight_per_term = expansion_weight / max(total_expansion, 1) if total_expansion > 0 else 0
    ...
    async def _get_from_cache(self, cache_key: str) -> Optional[ExpansionResult]:
        """キャッシュから取得"""
