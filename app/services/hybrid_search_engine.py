...
class SearchConfig:
    ...
    # Vector検索パラメータ
    dense_search_params: Dict[str, Any] = field(default_factory=lambda: {
        "metric_type": "IP",  # Inner Product
        "nprobe": 16,
    })
    sparse_search_params: Dict[str, Any] = field(default_factory=lambda: {
...
class SearchResult:
    ...
    search_time: float
    query: str
    facets: Optional[Dict[str, List[FacetResult]]] = None
...
class HybridSearchEngine:
    ...
    def __init__(
        ...
        chunk_repository: Optional[DocumentChunkRepository] = None,
    ):
        self.config = config
        ...
        try:
            ...
            if self.config.search_mode == SearchMode.HYBRID:
                fused_results = self._fuse_search_results(
                    search_results.get("dense"),
                    search_results.get("sparse"),
                    query
                )
            ...
            if self.config.similarity_threshold > 0:
                filtered_documents = self._filter_by_similarity_threshold(filtered_documents)
            
            # ページネーション
            paginated_documents = filtered_documents[query.offset:query.offset + query.max_results]
    ...
    def _fuse_search_results(
        ...
        if self.config.ranking_algorithm == RankingAlgorithm.RRF:
            if dense_result and sparse_result:
                ...
                return list(zip(dense_result.ids, dense_result.scores))
            elif sparse_result:
                return list(zip(sparse_result.ids, sparse_result.scores))
            else:
                return []
    ...
    def _process_single_search_result(
        ...
        """単一検索結果の処理"""
        if search_result:
            return list(zip(search_result.ids, search_result.scores))
        else:
            return []
    ...
    def _calculate_popularity_scores(
        ...
        # 最大値を取得してスケーリング
        view_counts = [doc.get("metadata", {}).get("view_count", 0) for doc in documents]
    ...
    def _calculate_quality_scores(
        ...
        # メトリクスの最大値を取得
        word_counts = [doc.get("metadata", {}).get("word_count", 0) for doc in documents]
    ...
    def _rerank_with_features(
        ...
        # 統合関連性スコアを計算
        relevance_scores = self._calculate_combined_relevance_scores(documents, query_text)
    ...
    def _optimize_ranking_pipeline(
        ...
        personalization_scores = {}
        if user_profile:
            personalization_scores = self._calculate_personalization_scores(documents, user_profile)
