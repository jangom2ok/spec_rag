...
class TestHybridSearchEngine:
    ...
    async def test_search_engine_initialization(
        self, basic_search_config: SearchConfig
    ):
        """検索エンジンの初期化テスト"""
    ...
    async def test_query_embedding_generation(
        self, basic_search_config: SearchConfig, sample_search_query: SearchQuery
    ):
        """クエリ埋め込み生成テスト"""
        search_engine = HybridSearchEngine(config=basic_search_config)

        with patch.object(
            search_engine, "_generate_query_embeddings", new_callable=AsyncMock
        ) as mock_embed:
            ...
            embeddings = await search_engine._generate_query_embeddings(
                sample_search_query.text
            )
    ...
    async def test_dense_vector_search(
        ...
        with patch.object(
            search_engine, "_search_dense_vectors", new_callable=AsyncMock
        ) as mock_search:
            ...
            result = await search_engine._search_dense_vectors(
                query_vector, sample_search_query
            )
    ...
    async def test_sparse_vector_search(
        ...
        with patch.object(
            search_engine, "_search_sparse_vectors", new_callable=AsyncMock
        ) as mock_search:
            mock_search.return_value = VectorSearchResult(
            ...
            result = await search_engine._search_sparse_vectors(
                query_vector, sample_search_query
            )
    ...
    async def test_semantic_search_mode(
        self, semantic_search_config: SearchConfig, sample_search_query: SearchQuery
    ):
        """セマンティック検索モードテスト"""
    ...
    async def test_keyword_search_mode(
        self, keyword_search_config: SearchConfig, sample_search_query: SearchQuery
    ):
        """キーワード検索モードテスト"""
    ...
    async def test_japanese_text_search(
        self, basic_search_config: SearchConfig, japanese_search_query: SearchQuery
    ):
        """日本語テキスト検索テスト"""
        search_engine = HybridSearchEngine(config=basic_search_config)
    ...
    @pytest.mark.unit
    async def test_search_result_scoring(self, basic_search_config: SearchConfig):
        """検索結果スコアリングテスト"""
        search_engine = HybridSearchEngine(config=basic_search_config)
    ...
    async def test_search_timeout_handling(
        ...
        with patch.object(
            ...
            assert (
                ...
                or result.search_time > basic_search_config.search_timeout
            )

    @pytest.mark.unit
    async def test_empty_query_handling(self, basic_search_config: SearchConfig):
        """空クエリ処理テスト"""
        search_engine = HybridSearchEngine(config=basic_search_config)
    ...
    async def test_end_to_end_search(
        ...
        with (
            ...
            for i in range(len(result.documents) - 1):
                assert (
                    result.documents[i]["search_score"]
                    >= result.documents[i + 1]["search_score"]
                )
...
class TestSearchResult:
    ...
    def test_search_result_creation(self):
        ...
        facets = {"source_type": [FacetResult(value="confluence", count=2)]}
