...
class TestAuthenticationDependency:
    ...
    async def test_jwt_authentication_blacklisted_token(self):
        ...
        with patch('app.api.search.is_token_blacklisted') as mock_blacklist, \
...
class TestSearchEngineDependency:
    ...
    async def test_get_hybrid_search_engine_initialization(self):
        ...
        with patch('app.services.embedding_service.EmbeddingService') as MockEmbedding, \
             patch('app.repositories.document_repository.DocumentRepository') as MockDocRepo, \
             patch('app.repositories.chunk_repository.DocumentChunkRepository') as MockChunkRepo:
...
class TestSearchEndpoints:
    ...
    async def test_search_documents_comprehensive(self, client, auth_headers):
        ...
        with patch('app.api.search.get_current_user_or_api_key') as mock_auth, \
            ...
            mock_engine.search = AsyncMock(return_value=SearchResult(
                success=True,
                query="test query",
                total_hits=2,
                search_time=0.5,
    ...
    async def test_search_documents_weight_normalization(self, client, auth_headers):
        ...
        with patch('app.api.search.get_current_user_or_api_key') as mock_auth, \
            ...
            response = client.post("/api/v1/search/", json=request_data, headers=auth_headers)
    ...
    async def test_search_documents_no_permission(self, client, auth_headers):
        ...
        with patch('app.api.search.get_current_user_or_api_key') as mock_auth:
            mock_auth.return_value = {"user_id": "test", "permissions": ["write"]}  # No read
    ...
    async def test_search_documents_search_failure(self, client, auth_headers):
        ...
        with patch('app.api.search.get_current_user_or_api_key') as mock_auth, \
            ...
            response = client.post("/api/v1/search/", json={"query": "test"}, headers=auth_headers)
    ...
    async def test_search_documents_exception(self, client, auth_headers):
        ...
        with patch('app.api.search.get_current_user_or_api_key') as mock_auth, \
            ...
            mock_engine = AsyncMock()
            mock_engine.config = SearchConfig()
            mock_engine.search = AsyncMock(side_effect=Exception("Database connection failed"))
            mock_get_engine.return_value = mock_engine
    ...
    async def test_semantic_search_endpoint(self, client, auth_headers):
        ...
        with patch('app.api.search.get_current_user_or_api_key') as mock_auth, \
            ...
            response = client.post("/api/v1/search/semantic", json=request_data, headers=auth_headers)
    ...
    async def test_semantic_search_failure(self, client, auth_headers):
        ...
        with patch('app.api.search.get_current_user_or_api_key') as mock_auth, \
            ...
            response = client.post("/api/v1/search/semantic", json={"query": "test"}, headers=auth_headers)
    ...
    async def test_keyword_search_endpoint(self, client, auth_headers):
        ...
        with patch('app.api.search.get_current_user_or_api_key') as mock_auth, \
            ...
            response = client.post("/api/v1/search/keyword", json={"query": "keyword test"}, headers=auth_headers)
    ...
    async def test_search_suggestions_endpoint(self, client, auth_headers):
        ...
        with patch('app.api.search.get_current_user_or_api_key') as mock_auth:
            ...
            response = client.get(
                "/api/v1/search/suggestions?q=machine&limit=3",
                headers=auth_headers
            )
    ...
    async def test_search_suggestions_no_permission(self, client, auth_headers):
        ...
        with patch('app.api.search.get_current_user_or_api_key') as mock_auth:
            ...
            response = client.get("/api/v1/search/suggestions?q=test", headers=auth_headers)
    ...
    async def test_search_config_exception(self, client, auth_headers):
        ...
        with patch('app.api.search.get_current_user_or_api_key') as mock_auth:
            ...
            with patch('app.api.search.SearchMode', side_effect=Exception("Enum error")):
                response = client.get("/api/v1/search/config", headers=auth_headers)
    ...
    async def test_search_keyword_alias_endpoint(self, client, auth_headers):
        ...
        with patch('app.api.search.search_documents') as mock_search_documents:
            ...
            response = client.post("/api/v1/search/keyword", json=request_data, headers=auth_headers)
...
class TestSearchModeConfiguration:
    ...
    async def test_search_mode_dense(self):
        ...
        with patch('app.api.search.get_current_user_or_api_key') as mock_auth, \
            ...
            response = client.post("/api/v1/search/", json=request_data, headers={"X-API-Key": "test"})
