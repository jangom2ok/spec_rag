...
class TestSearchFiltering:
    ...
    @pytest.mark.unit
    def test_single_field_exact_filter(self, search_config: SearchConfig, sample_documents: List[Dict[str, Any]]):
        """単一フィールド完全一致フィルター"""
    ...
    @pytest.mark.unit
    def test_nested_field_filter(self, search_config: SearchConfig, sample_documents: List[Dict[str, Any]]):
        """ネストされたフィールドフィルター"""
        search_engine = HybridSearchEngine(config=search_config)
        
        # metadata.category = "development" でフィルタリング
        filters = [SearchFilter(field="metadata.category", value="development", operator="eq")]
        
        filtered_docs = search_engine._apply_query_filters(sample_documents, filters)
        
        assert len(filtered_docs) == 2  # doc-1, doc-4
        assert all(doc["metadata"]["category"] == "development" for doc in filtered_docs)

    @pytest.mark.unit
    def test_multiple_filters_and_condition(self, search_config: SearchConfig, sample_documents: List[Dict[str, Any]]):
        """複数フィルター（AND条件）"""
        search_engine = HybridSearchEngine(config=search_config)
        ...
        assert len(filtered_docs) == 2  # doc-1, doc-4
        assert all(doc["language"] == "en" and doc["document_type"] == "guide" for doc in filtered_docs)

    @pytest.mark.unit
    def test_not_equal_filter(self, search_config: SearchConfig, sample_documents: List[Dict[str, Any]]):
        """不等フィルター"""
        search_engine = HybridSearchEngine(config=search_config)
    ...
    @pytest.mark.unit
    def test_in_operator_filter(self, search_config: SearchConfig, sample_documents: List[Dict[str, Any]]):
        """IN演算子フィルター"""
        search_engine = HybridSearchEngine(config=search_config)
        
        # document_type in ["guide", "tutorial"]
        filters = [SearchFilter(field="document_type", value=["guide", "tutorial"], operator="in")]
        
        filtered_docs = search_engine._apply_query_filters(sample_documents, filters)
        
        assert len(filtered_docs) == 4  # doc-1, doc-2, doc-4, doc-5
        assert all(doc["document_type"] in ["guide", "tutorial"] for doc in filtered_docs)

    @pytest.mark.unit
    def test_not_in_operator_filter(self, search_config: SearchConfig, sample_documents: List[Dict[str, Any]]):
        """NOT IN演算子フィルター"""
        search_engine = HybridSearchEngine(config=search_config)
        
        # metadata.difficulty not in ["beginner", "advanced"]
        filters = [SearchFilter(field="metadata.difficulty", value=["beginner", "advanced"], operator="not_in")]
        
        filtered_docs = search_engine._apply_query_filters(sample_documents, filters)
        
        assert len(filtered_docs) == 2  # doc-3, doc-4 (intermediate)
        assert all(doc["metadata"]["difficulty"] == "intermediate" for doc in filtered_docs)

    @pytest.mark.unit
    def test_range_filters_numeric(self, search_config: SearchConfig, sample_documents: List[Dict[str, Any]]):
        """数値範囲フィルター"""
        search_engine = HybridSearchEngine(config=search_config)
    ...
    @pytest.mark.unit
    def test_array_field_filter(self, search_config: SearchConfig, sample_documents: List[Dict[str, Any]]):
        """配列フィールドフィルター"""
        search_engine = HybridSearchEngine(config=search_config)
    ...
    @pytest.mark.unit
    def test_similarity_threshold_filter(self, search_config: SearchConfig, sample_documents: List[Dict[str, Any]]):
        """類似度閾値フィルター"""
    ...
    def test_nested_field_extraction(self, search_config: SearchConfig):
        ...
        # 存在しないパス
        missing_field = search_engine._get_nested_field(document, "metadata.nonexistent.field")
...
class TestSearchFacets:
    ...
    @pytest.mark.unit
    def test_single_facet_calculation(self, search_config: SearchConfig, facet_test_documents: List[Dict[str, Any]]):
        """単一ファセット計算"""
        search_engine = HybridSearchEngine(config=search_config)
        ...
        confluence_facet = next(f for f in source_type_facets if f.value == "confluence")
    ...
    @pytest.mark.unit
    def test_multiple_facets_calculation(self, search_config: SearchConfig, facet_test_documents: List[Dict[str, Any]]):
        """複数ファセット計算"""
        search_engine = HybridSearchEngine(config=search_config)
        
        facets = search_engine._calculate_facets(
            facet_test_documents, 
            ["source_type", "language", "document_type"]
        )
    ...
    @pytest.mark.unit
    def test_nested_field_facets(self, search_config: SearchConfig, facet_test_documents: List[Dict[str, Any]]):
        """ネストフィールドファセット"""
        search_engine = HybridSearchEngine(config=search_config)
        
        facets = search_engine._calculate_facets(facet_test_documents, ["metadata.category"])
    ...
    @pytest.mark.unit
    def test_array_field_facets(self, search_config: SearchConfig, facet_test_documents: List[Dict[str, Any]]):
        """配列フィールドファセット"""
        search_engine = HybridSearchEngine(config=search_config)
        
        facets = search_engine._calculate_facets(facet_test_documents, ["metadata.tags"])
    ...
    @pytest.mark.unit
    def test_facet_sorting(self, search_config: SearchConfig, facet_test_documents: List[Dict[str, Any]]):
        """ファセットソート（頻度順）"""
        search_engine = HybridSearchEngine(config=search_config)
        
        facets = search_engine._calculate_facets(facet_test_documents, ["document_type"])
    ...
    def test_combined_filter_and_facets(self, search_config: SearchConfig, facet_test_documents: List[Dict[str, Any]]):
        """フィルターとファセットの組み合わせ"""
        search_engine = HybridSearchEngine(config=search_config)
        
        # 英語ドキュメントのみでファセット計算
        filters = [SearchFilter(field="language", value="en", operator="eq")]
        filtered_docs = search_engine._apply_query_filters(facet_test_documents, filters)
