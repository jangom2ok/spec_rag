...
- インフラ：AWS、CloudFormation""",
            "source_type": "markdown",
            "file_type": "md",
            "metadata": {"author": "system_analyst", "department": "IT", "version": "2.1"},
        }
    ...
    @pytest.mark.unit
    async def test_basic_metadata_extraction(self, basic_config: ExtractionConfig, sample_markdown_document: Dict[str, Any]):
        """基本メタデータ抽出テスト"""
        extractor = MetadataExtractor(config=basic_config)
    ...
    @pytest.mark.unit
    async def test_language_detection(self, basic_config: ExtractionConfig, sample_japanese_document: Dict[str, Any]):
        """言語検出テスト"""
        extractor = MetadataExtractor(config=basic_config)
    ...
    @pytest.mark.unit
    async def test_structure_extraction_markdown(self, basic_config: ExtractionConfig, sample_markdown_document: Dict[str, Any]):
        """Markdown構造抽出テスト"""
        extractor = MetadataExtractor(config=basic_config)
    ...
    @pytest.mark.unit
    async def test_structure_extraction_html(self, basic_config: ExtractionConfig, sample_confluence_document: Dict[str, Any]):
        """HTML構造抽出テスト"""
        extractor = MetadataExtractor(config=basic_config)
    ...
    @pytest.mark.unit
    async def test_entity_extraction(self, basic_config: ExtractionConfig, sample_markdown_document: Dict[str, Any]):
        """エンティティ抽出テスト"""
        extractor = MetadataExtractor(config=basic_config)
    ...
    @pytest.mark.unit
    async def test_keyword_extraction(self, basic_config: ExtractionConfig, sample_markdown_document: Dict[str, Any]):
        """キーワード抽出テスト"""
        extractor = MetadataExtractor(config=basic_config)
    ...
    @pytest.mark.unit
    async def test_statistics_extraction(self, basic_config: ExtractionConfig, sample_markdown_document: Dict[str, Any]):
        """統計情報抽出テスト"""
        extractor = MetadataExtractor(config=basic_config)
    ...
    async def test_confidence_filtering(self):
        ...
        if "entities" in high_result.metadata and "entities" in low_result.metadata:
            high_entities = len(high_result.metadata["entities"].get("technical_terms", []))
            low_entities = len(low_result.metadata["entities"].get("technical_terms", []))
    ...
    @pytest.mark.unit
    async def test_metadata_field_types(self, basic_config: ExtractionConfig, sample_markdown_document: Dict[str, Any]):
        """メタデータフィールドタイプテスト"""
        extractor = MetadataExtractor(config=basic_config)
    ...
    async def test_incremental_extraction(self, basic_config: ExtractionConfig):
        ...
        if "existing_metadata" in result.metadata:
            assert result.metadata["existing_metadata"]["custom_field"] == "custom_value"
