...
class TestMockClassesMissing:
    ...
    def test_mock_async_result_states(self):
        ...
        # Test SUCCESS state
        success_result = MockAsyncResult("task2", state="SUCCESS", result={"data": "value"})
        ...
        # Test FAILURE state
        failure_result = MockAsyncResult("task3", state="FAILURE", result=Exception("Error"))
...
class TestEmbeddingTaskServiceMissing:
    ...
    async def test_initialize_with_logging(self):
        ...
        with patch("app.services.embedding_tasks.EmbeddingConfig") as mock_config:
            with patch("app.services.embedding_tasks.EmbeddingService") as mock_embedding:
                with patch("app.services.embedding_tasks.ChunkRepository") as mock_repo:
                    with patch("app.services.embedding_tasks.logger") as mock_logger:
                        ...
                        # Verify logging
                        mock_logger.info.assert_called_with("EmbeddingTaskService initialized")
    ...
    async def test_process_batch_texts_error(self):
        ...
        service = EmbeddingTaskService()
        service.embedding_service = AsyncMock()
        service.embedding_service.generate_embeddings.side_effect = Exception("Embedding failed")
...
class TestTaskServiceSingleton:
    ...
    async def test_get_task_service_initialization(self):
        ...
        with patch.object(EmbeddingTaskService, 'initialize', new_callable=AsyncMock) as mock_init:
...
class TestHealthCheckFunctions:
    ...
    def test_get_celery_health_all_cases(self):
        ...
        with patch.object(celery_app.control, 'inspect', return_value=mock_inspect_empty):
            result = get_celery_health()
        ...
        # Test connection error
        with patch.object(celery_app.control, 'inspect', side_effect=Exception("Broker error")):
            result = get_celery_health()
...
class TestEmbeddingTaskManager:
    ...
    def test_get_worker_status_with_errors(self):
        ...
        # Test with exception
        with patch("app.services.embedding_tasks.HAS_CELERY", True):
            with patch.object(celery_app.control, 'inspect', side_effect=Exception("Error")):
                status = EmbeddingTaskManager.get_worker_status()
    ...
    def test_retry_failed_tasks(self):
        ...
        with patch("app.services.embedding_tasks.HAS_CELERY", True):
            ...
            with patch("app.services.embedding_tasks.process_document_embedding_task", mock_task):
