...
class TestRerankerService:
    ...
    @pytest.mark.unit
    async def test_reranker_service_initialization(self, basic_reranker_config: RerankerConfig):
        """Rerankerサービス初期化テスト"""
    ...
    async def test_cross_encoder_reranking(
        ...
        with patch.object(reranker, '_get_cross_encoder_scores', new_callable=AsyncMock) as mock_scores:
            # CrossEncoderスコアをモック
            ...
            assert result.documents[0]["rerank_score"] > result.documents[0]["search_score"]
    ...
    async def test_colbert_reranking(
        ...
        with patch.object(reranker, '_get_colbert_scores', new_callable=AsyncMock) as mock_scores:
            # ColBERTスコアをモック
    ...
    async def test_ensemble_reranking(
        ...
        with patch.object(reranker, '_get_ensemble_scores', new_callable=AsyncMock) as mock_ensemble:
            # アンサンブルスコアを直接モック
    ...
    async def test_reranking_with_explanations(
        ...
        with patch.object(reranker, '_get_cross_encoder_scores', new_callable=AsyncMock) as mock_scores, \
             patch.object(reranker, '_generate_explanations', new_callable=AsyncMock) as mock_explain:
            
            mock_scores.return_value = [0.91, 0.87, 0.83, 0.74, 0.69]
            mock_explain.return_value = [
                {"relevance_factors": ["title_match", "content_relevance"], "confidence": 0.91},
                {"relevance_factors": ["content_match", "semantic_similarity"], "confidence": 0.87},
            ]
    ...
    async def test_reranking_cache_functionality(
        ...
        with patch.object(reranker, '_get_cache_key') as mock_cache_key, \
             patch.object(reranker, '_get_from_cache', new_callable=AsyncMock) as mock_get_cache, \
             patch.object(reranker, '_set_cache', new_callable=AsyncMock) as mock_set_cache, \
             patch.object(reranker, '_get_cross_encoder_scores', new_callable=AsyncMock) as mock_scores:
    ...
    async def test_reranking_timeout_handling(
        ...
        with patch.object(reranker, '_get_cross_encoder_scores', new_callable=AsyncMock) as mock_scores:
            ...
            assert result.success is False or result.rerank_time > basic_reranker_config.timeout
    ...
    async def test_reranker_performance_benchmarking(
        ...
        with patch.object(reranker, '_get_cross_encoder_scores', new_callable=AsyncMock) as mock_scores:
            mock_scores.return_value = [0.9 - (i * 0.001) for i in range(len(large_dataset))]
...
class TestColBERTReranker:
    ...
    async def test_colbert_maxsim_scoring(self):
        ...
        with patch.object(reranker, '_encode_query') as mock_encode_q, \
...
class TestEnsembleReranker:
    ...
    async def test_ensemble_score_combination(self):
        ...
        for actual, expect in zip(ensemble_scores, expected):
