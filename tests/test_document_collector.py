...
class TestDocumentCollector:
    ...
    @pytest.mark.unit
    async def test_collect_documents_from_test_source(
        self, mock_collector: DocumentCollector
    ):
        ...
        with patch.object(
            mock_collector, "_fetch_from_source", new_callable=AsyncMock
        ) as mock_fetch:
    ...
    @pytest.mark.unit
    async def test_collect_documents_with_batch_processing(
        self, mock_collector: DocumentCollector
    ):
        """バッチ処理での収集テスト"""
    ...
    @pytest.mark.unit
    async def test_collect_documents_with_errors(
        self, mock_collector: DocumentCollector
    ):
        """エラーがある場合の収集テスト"""
        with patch.object(
    ...
    @pytest.mark.unit
    async def test_collect_documents_with_filters(
        self, mock_collector: DocumentCollector
    ):
        """フィルター適用での収集テスト"""
