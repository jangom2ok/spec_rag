...
class TestMockEndpoints:
    ...
    async def test_create_document_mock(self):
        ...
        doc_create = DocumentCreate(
            title="Test Title", content="Test Content", source_type=SourceType.jira
        )
    ...
    async def test_create_document_no_write_permission(self):
        ...
        doc_create = DocumentCreate(
            title="Test Title", content="Test Content", source_type=SourceType.test
        )
    ...
    async def test_delete_document_mock(self):
        ...
        result = await delete_document(document_id="test-doc-123", current_user=user)
        ...
        result = await delete_document(
            document_id="admin-doc-456", current_user=admin_user
        )
    ...
    async def test_delete_document_no_permission(self):
        ...
        with pytest.raises(HTTPException) as exc_info:
            await delete_document(document_id="no-perm-doc", current_user=user)
    ...
    async def test_get_document_mock(self):
        ...
        result = await get_document(document_id="test-id", current_user=user)
    ...
    async def test_get_document_not_found(self):
        ...
        with pytest.raises(HTTPException) as exc_info:
            await get_document(document_id="non-existent-doc", current_user=user)
