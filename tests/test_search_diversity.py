...
class TestSearchDiversityService:
    ...
    async def test_mmr_diversification(
        ...
        with patch.object(diversity_service, '_diversify_with_mmr', new_callable=AsyncMock) as mock_mmr:
    ...
    async def test_clustering_diversification(
        ...
        with patch.object(diversity_service, '_diversify_with_clustering', new_callable=AsyncMock) as mock_clustering:
    ...
    async def test_topic_based_diversification(
        ...
        with patch.object(diversity_service, '_diversify_with_topics', new_callable=AsyncMock) as mock_topics:
            mock_topics.return_value = [
                diversification_request.candidates[0],  # machine_learning トピック
                diversification_request.candidates[2],  # natural_language_processing トピック
    ...
    async def test_temporal_diversification(
        ...
        with patch.object(diversity_service, '_diversify_with_temporal', new_callable=AsyncMock) as mock_temporal:
    ...
    async def test_diversification_execution(
        ...
        # すべての多様化手法をモック
        with patch.object(diversity_service, '_diversify_with_mmr', new_callable=AsyncMock) as mock_mmr, \
             patch.object(diversity_service, '_calculate_similarity_matrix', new_callable=AsyncMock) as mock_similarity:
    ...
    async def test_similarity_matrix_calculation(
        ...
        similarity_matrix = await diversity_service._calculate_similarity_matrix(sample_candidates)
        
        assert similarity_matrix.shape == (len(sample_candidates), len(sample_candidates))
    ...
    async def test_mmr_score_calculation(
        ...
        with patch.object(mmr_diversifier, '_calculate_similarity') as mock_similarity:
            ...
            expected_score = diversity_factor * candidate.score - (1 - diversity_factor) * 0.7
    ...
    async def test_clustering_candidates(
        ...
        clustering_diversifier = diversity_service.diversifiers[DiversificationAlgorithm.CLUSTERING]
        
        with patch.object(clustering_diversifier, '_cluster_candidates', new_callable=AsyncMock) as mock_cluster:
    ...
    async def test_topic_extraction_and_analysis(
        ...
        topic_diversifier = diversity_service.diversifiers[DiversificationAlgorithm.TOPIC_BASED]
        
        with patch.object(topic_diversifier, '_extract_topics', new_callable=AsyncMock) as mock_topics:
            mock_topics.return_value = {
            ...
            # トピック多様性スコア計算
            diversity_score = topic_diversifier._calculate_topic_diversity_score(topics_dict)
    ...
    async def test_diversity_criteria_filtering(
        ...
        for candidate_id, scores in diversity_scores.items():
    ...
    async def test_diversification_caching(
        ...
        with patch.object(diversity_service, '_get_cache_key') as mock_cache_key, \
             patch.object(diversity_service, '_get_from_cache', new_callable=AsyncMock) as mock_get_cache, \
             patch.object(diversity_service, '_set_cache', new_callable=AsyncMock) as mock_set_cache, \
             patch.object(diversity_service, '_diversify_with_mmr', new_callable=AsyncMock) as mock_mmr:
    ...
    async def test_end_to_end_diversification(
        ...
        # 完全な多様化パイプラインのモック
        with patch.object(diversity_service, '_calculate_similarity_matrix', new_callable=AsyncMock) as mock_similarity, \
             patch.object(diversity_service, '_diversify_with_mmr', new_callable=AsyncMock) as mock_mmr:
            ...
            # 多様性が向上していることを確認
            original_categories = [c.metadata["category"] for c in diversification_request.candidates]
            diversified_categories = [c.metadata["category"] for c in result.diversified_candidates]
...
class TestMMRDiversifier:
    ...
    async def test_mmr_selection_algorithm(self):
        ...
        # サンプル候補と類似度行列
        candidates = [
            DiversityCandidate(id=f"doc-{i}", content=f"content {i}", title=f"title {i}", 
                             score=0.9-i*0.1, embedding=np.random.random(10))
            for i in range(5)
        ]
        ...
        with patch.object(diversifier, '_calculate_similarity_matrix') as mock_similarity:
            mock_similarity.return_value = similarity_matrix
            
            selected = await diversifier.select(candidates, max_results=3, diversity_factor=0.5)
...
class TestClusteringDiversifier:
    ...
    async def test_kmeans_clustering(self):
        ...
        candidates = [
            DiversityCandidate(id=f"doc-{i}", content=f"content {i}", title=f"title {i}",
                             score=0.9, embedding=np.random.random(10))
            for i in range(6)
        ]
...
class TestTopicDiversifier:
    ...
    async def test_topic_extraction(self):
        ...
        with patch.object(diversifier, '_extract_topics_from_content') as mock_extract:
            mock_extract.return_value = ["machine_learning", "algorithms", "neural_networks"]
            
            topics = await diversifier._extract_topics_from_content("machine learning algorithms")
...
class TestTemporalDiversifier:
    ...
    async def test_temporal_distribution(self):
        ...
        selected = await diversifier.select_temporally_diverse(candidates, max_results=3, window_days=7)
